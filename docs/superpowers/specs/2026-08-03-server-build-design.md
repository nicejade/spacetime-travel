...
**Date:** 2026-08-03  
**Status:** Approved  
**Scope:** Compile Fastify server (+ shared) to JS; production entry uses `node dist/server/index.js`

## Goal

Add a `build:server` script that TypeScript-compiles `server/` and its `shared/` imports into `dist/`, and switch production run paths (`start`, PM2, Docker) from `tsx server/index.ts` to `node dist/server/index.js`.
...
Dedicated `tsconfig.server.json` with emit enabled; root `tsconfig.json` stays `noEmit` for editor/`typecheck`. Production entrypoints consume compiled output only.
...
| `outDir` | `dist` | Emit `dist/server/` + `dist/shared/` |
...
Resolve `publicPath` and `defaultDbPath` relative to the **repo root**, whether the file runs from `server/` (tsx) or `dist/server/` (node), so Vite output stays at `server/public/` and SQLite default at `server/data/`.
...
- Production: copy `dist/`, keep `server/public` (and data dir layout); `CMD ["node", "dist/server/index.js"]`
...
3. `pnpm build:server` (API → `dist/`)
4. `pnpm start` or `pnpm deploy` (PM2)

## Error handling

- If `dist/` is missing, `node` / PM2 fail with module-not-found — operator must run `build:server`
- `tsc` failures abort `build:server` with non-zero exit

## Testing

- `pnpm build:server` exits 0; `dist/server/index.js` exists
- `pnpm build && pnpm build:server && pnpm start` → `GET /api/health` returns `{"ok":true}`
- `pnpm api` still starts via tsx without requiring `dist/`
